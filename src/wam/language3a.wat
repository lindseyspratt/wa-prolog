(;
Language3A is Language3 with optimizations as described in Chapter 5 sections 1 through 4:
    Heap Representation (change to put_structure instruction);
    Constants, lists, and anonymous variables (add put_constant, get_constant, set_constant, unify_constant,
    put_list, get_list, set_void, and unify_void instructions;
    add LIS and CON tags for lists and constants, respectively);
    A note on 'set' instructions (no changes in this section); and
    Register allocation (no changes to language3, but changes in compilation).
...
;;    )
   ...
   (func $storeRegisterToHeapTop (param $reg i32)
    ...
    (func $create_environment_frame (param $newE i32) (param $n i32)
    ...
    (func $allocate (param $N i32)
        ...
        (call $create_environment_frame (local.get $newE) (local.get $N))
