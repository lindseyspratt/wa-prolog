/*
These utilities are an essential accompaniment to the WAM implemented by wam.wat.
The importObject defines the memory layout of the WAM and the Javascript utility functions
available to the WAM WebAssembly code.
These utility functions provide:
access to the arity of a predicate (getIndicatorArity),
access to the 'code' words that define Prolog programs (getCodeFromProgram, getCode, setCode),
display of trace logging,
and display of warning messages.
 */
...
The memory areas from low to high: PDL, Trail, Heap, Stack, Registers
...
function runQuery(limit, queryCode, obj) {
    ...
    for(let i = 0; i < MIN_REGISTER + 256;i++) {
        i32Initial[i] = 0;
    }
    let start = Date.now();
...
function runOp(evalCodes, obj) {
    let i32Initial = new Uint32Array(memory.buffer);
    for(let i = 0; i < MIN_REGISTER + 256;i++) {
        i32Initial[i] = 0;
    }
    code = evalCodes;
    obj.instance.exports.runOp();
}
...
function initialize_op_codes(obj) {
    ...
    opCodes.put_atom = obj.instance.exports.put_atom_opcode();
    opCodes.get_atom = obj.instance.exports.get_atom_opcode();
    opCodes.set_atom = obj.instance.exports.set_atom_opcode();
    opCodes.unify_atom = obj.instance.exports.unify_atom_opcode();
    opCodes.put_integer = obj.instance.exports.put_integer_opcode();
    opCodes.get_integer = obj.instance.exports.get_integer_opcode();
    opCodes.set_integer = obj.instance.exports.set_integer_opcode();
    opCodes.unify_integer = obj.instance.exports.unify_integer_opcode();
    opCodes.put_float = obj.instance.exports.put_float_opcode();
    opCodes.get_float = obj.instance.exports.get_float_opcode();
    opCodes.set_float = obj.instance.exports.set_float_opcode();
    opCodes.unify_float = obj.instance.exports.unify_float_opcode();
    ...
    opCodes.halt = obj.instance.exports.halt_opcode();
}
...
function log_results(test, length) {
    let results = new Uint32Array(memory.buffer);
    console.log('Test: ' + test);
    console.log('startingWord=' + MIN_HEAP + ': ' + condensed_array(results.slice(MIN_HEAP, MIN_HEAP+length)));
    console.log('registers: ' + interpret_memory(results, MIN_REGISTER, MIN_REGISTER+length));
    console.log('heap: ' + interpret_memory(results, MIN_HEAP, MIN_HEAP+length));
}

function condensed_array(values) {
    let previousInfoString = '';
    let start = -1;
    let end = -1;
    let output = '';
    for(let i = 0; i < values.length;i++) {
        if(start === -1) {
            ...
            previousInfoString = values[i];

        } else if(previousInfoString === values[i]) {
            end = i;
        } else {
            if(start === end) {
                output += start + ': ' + previousInfoString + '; ';
            } else {
                output += start + '-' + end + ': ' + previousInfoString + '; ';
            }
            start = i;
            end = i;
            previousInfoString = values[i];
...
function interpret_memory(results, startingWord, explicitLength) {
    ...
    while(i < results.length) {
        ...
        if(start === -1) {
            ...
            previousInfoString = info.string;

        } else if(previousInfoString === info.string) {
            end = i;
        } else {
            if(start === end) {
                output += start + ': ' + previousInfoString + '; ';
...
function interpret_memory_item(results, itemOfst) {
    ...
    if(word === 0) {
        return {string:'<0>', nextItemOfst:itemOfst+1};
    }
    ...
    switch(tag) {
        ...
        case TAG_ATM: // AToM: 'a' instead of a/0.
        {
        ...
        case TAG_INT: // INTeger.
        {
            let result = get_val(word);
            return {string: result, nextItemOfst: itemOfst+1};
        }
        case TAG_FLT: // FLoaT.
        {
            let internalID = get_val(word);
            let float = find_float(internalID);
            let result = '';
            result +=  "'" + float + "'";
            return {string: result, nextItemOfst: itemOfst+1};
        }
...
function traceInstLog6 (opCode, arg1, arg2, arg3, arg4, arg5, arg6) {
    let opName = getOpCodeName(opCode);
    console.log(traceInstPrefix(6) + opName + " " + arg1 + ", " + arg2 + ", " + arg3 + ", " + arg4 + ", " + arg5 + ", " + arg6 + ";");
}
...
const TAG_ATM = 3; // 0x18000000 // was TAG_ATM.
const TAG_INT = 4; // 0x20000000
...
module.exports.log_results = log_results;
