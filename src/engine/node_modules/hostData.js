...
function getIndicatorWA(name, arity) {
    return lookupIndicator(name, arity);
}
...
function foreignWA(pred, ...args) {
    ...
    foreignFunction.apply(null, args);
}

function lookup_atom(name, noCreation)
{
...
function get_atom_id(name) {
    return lookup_atom(name ,true);
}
...
function get_indicator_id(name, arity) {
    return lookupIndicator(name, arity, true);
}

function lookupIndicator(name, arity, noCreation) {
    if(typeof name !== 'string') {
        throw 'invalid name "' + name + '" for call lookupIndicator(' + name + ', ' + arity + ', ' + noCreation + ')';
    }

    if(typeof arity !== 'number') {
        throw 'invalid arity "' + arity + '" for call lookupIndicator(' + name + ', ' + arity + ', ' + noCreation + ')';
    }

    if(typeof noCreation !== 'boolean' && typeof noCreation !== 'undefined') {
        throw 'invalid noCreation "' + noCreation + '" for call lookupIndicator(' + name + ', ' + arity + ', ' + noCreation + ')';
    }

    let nameID = lookup_atom(name, noCreation);
...
function registerForeign(predicateName, arity, foreignFunction) {
    let indicator = lookupIndicator(predicateName, arity);
    foreigns[indicator] = foreignFunction;
}
...
module.exports.get_indicator_id = get_indicator_id;
