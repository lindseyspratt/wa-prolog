...
const HEAP_SIZE = Math.trunc(hostData.memory.buffer.byteLength/4) - (PDL_SIZE + TRAIL_SIZE + STACK_SIZE + REGISTER_SIZE);
...
let importObject = {js:
        {mem: hostData.memory,
            ...
            heapSize: HEAP_SIZE,
            getCodeFromProgram: hostData.getCodeFromProgramWA,
            getCode: hostData.getCodeWA,
            setCode: hostData.setCodeWA,
            getIndicatorArity: hostData.getIndicatorArityWA,
            getIndicator: hostData.get_indicator_id,
            foreign0: hostData.foreignWA,
            foreign1: hostData.foreignWA,
            foreign2: hostData.foreignWA,
            foreign3: hostData.foreignWA,
            foreign4: hostData.foreignWA,
            foreign5: hostData.foreignWA,
            ...
            traceTrailTR: traceTrailTR,
            traceTidyTrailTR: traceTidyTrailTR,
            traceInitializeGlobalsTR: traceInitializeGlobalsTR,
            traceRetryMeElseTR: traceRetryMeElseTR,
            traceTrustMeTR: traceTrustMeTR,
            traceRetryTR: traceRetryTR,
...
function runQuery(limit, queryCode, obj) {
    let inferences = 0;
    let i32Initial = new Uint32Array(hostData.memory.buffer);
    for(let i = 0; i < MIN_REGISTER + 256;i++) {
        i32Initial[i] = 0;
    }
    let start = Date.now();
    for(let i = 0;i < limit;i++) {
        hostData.set_code(queryCode);
...
function setupForeigns(commonHostData, obj) {
    foreign.setHostData(commonHostData);
    foreign.setWAM(obj);
    hostData.registerForeign('load_buffer', 1, foreign.predicate_load_buffer);
    hostData.registerForeign('get_byte', 1, foreign.predicate_get_byte);
}
...
function initialize_op_codes(obj) {
    ...
    opCodes.call_foreign = obj.instance.exports.call_foreign_opcode();
}
...
function display_results(test, length) {
    let results = new Uint32Array(hostData.memory.buffer);
    let root = document.getElementById('testResults');
    let testElement = document.createElement('li');

    appendDivText(testElement, 'Test: ' + test);
    appendDivText(testElement, 'startingWord=' + MIN_HEAP + ': ' + JSON.stringify(results.slice(MIN_HEAP, MIN_HEAP+length)));
    appendDivText(testElement, 'registers: ' + interpret_memory(results, MIN_REGISTER, MIN_REGISTER+length));
    appendDivText(testElement, 'heap: ' + interpret_memory(results, MIN_HEAP, MIN_HEAP+length));
    root.appendChild(testElement);
}

function appendDivText(node, text) {
...
function interpret_memory_item(results, itemOfst) {
    ...
    switch(tag) {
        ...
        case TAG_ATM: // AToM: 'a' instead of a/0.
        {
            ...
            let constant = hostData.find_atom(internalID);
            let result = '';
            result +=  "'" + constant + "'";
        ...
        {
            let internalID = get_val(word);
            let float = hostData.find_float(internalID);
            let result = '';
            result +=  "'" + float + "'";
            return {string: result, nextItemOfst: itemOfst+1};
        }
...
function interpret_memory_structure(results, result, indicator, structureWordOfst, itemOfst) {
    let pair = hostData.get_indicator_pair(indicator);
    let functorID = pair[0];
    result += hostData.find_atom(functorID) + '(';
    let arity = pair[1];
...
function traceInstPrefix(argCount) {
    return 'inst: ' + hostData.getCurrentIndicator() + '.' + (hostData.getCurrentWord()-argCount) + ' ';
}
...
function traceTrailTR(addr) {
    console.log ('    trail TR = ' + addr + '.')
}

function traceTidyTrailTR(addr) {
    console.log ('    tidy_trail TR = ' + addr + '.')
}

function traceInitializeGlobalsTR(addr) {
    console.log ('    initialize_globals TR = ' + addr + '.')
}

function traceRetryMeElseTR(addr) {
    console.log ('    retry_me_else TR = ' + addr + '.')
}

function traceTrustMeTR(addr) {
    console.log ('    trust_me TR = ' + addr + '.')
}

function traceRetryTR(addr) {
    console.log ('    retry TR = ' + addr + '.')
}
...
module.exports.lookup_atom = hostData.lookup_atom;
module.exports.find_atom = hostData.find_atom;
module.exports.lookup_float = hostData.lookup_float;
module.exports.find_float = hostData.find_float;
module.exports.lookupIndicator = hostData.lookupIndicator;
module.exports.registerProgram = hostData.registerProgram;
