(;
wam.wat is the WebAssembly implementation of a Warren Abstract Machine based on the book
"Warren's Abstract Machine: A Tutorial Reconstruction" by Hassan Aït-Kaci.
This implementation is an extension of tutorial/wam/language3e.wat.
...
(module
    ...
    (import "js" "traceInstLog6" (func $traceInstLog6 (param i32) (param i32) (param i32) (param i32) (param i32) (param i32) (param i32)))
...
;;    (import "js" "consoleLog" (func $consoleLog (param i32)))
    ...
    (global $TAG_ATM i32 (i32.const 3))
    ...
    (func $isCon (param $value i32) (result i32)
        (return
            (i32.or (i32.eq (global.get $TAG_ATM) (local.get $value))
                    (i32.or (i32.eq (global.get $TAG_INT) (local.get $value))
                        (i32.eq (global.get $TAG_FLT) (local.get $value))
                    )
            )
        )
    )
...
;;    )
    ...
    (func $tagAtom (param $val i32) (result i32)
        global.get $TAG_ATM
        local.get $val
        call $applyTag
    )
    ...
    (func $tagFloat (param $val i32) (result i32)
        global.get $TAG_FLT
        local.get $val
        call $applyTag
    )
    ...
    (func $storeConstantAtHeapTop (param $t i32) (param $c i32)
        (call $storeToHeap (global.get $H) (call $applyTag (local.get $t) (local.get $c)))
    )
...
;)
    ...
    (func $put_atom_opcode (result i32) (i32.const 18))
    (func $put_integer_opcode (result i32) (i32.const 39))
    (func $put_float_opcode (result i32) (i32.const 40))
    (func $get_atom_opcode (result i32) (i32.const 19))
    (func $get_integer_opcode (result i32) (i32.const 41))
    (func $get_float_opcode (result i32) (i32.const 42))
    (func $set_atom_opcode (result i32) (i32.const 20))
    (func $set_integer_opcode (result i32) (i32.const 43))
    (func $set_float_opcode (result i32) (i32.const 44))
    (func $unify_atom_opcode (result i32) (i32.const 21))
    (func $unify_integer_opcode (result i32) (i32.const 45))
    (func $unify_float_opcode (result i32) (i32.const 46))
    ...
    (func $evalOp (param $op i32)
        ...
        (block
            ...
            return
             ...
             (call $op39) ;; $put_integer
             return
        ) ;; 40
             (call $op40) ;; $put_float
             return
        ) ;; 41
             (call $op41) ;; $get_integer
             return
        ) ;; 42
             (call $op42) ;; $get_float
             return
        ) ;; 43
             (call $op43) ;; $set_integer
             return
        ) ;; 44
             (call $op44) ;; $set_float
             return
        ) ;; 45
             (call $op45) ;; $unify_integer
             return
        ) ;; 46
             (call $op46) ;; $unify_float
             return
        ) ;; 47
             (call $op47) ;; $halt
             return
    ...
    (func $traceInst6 (param $inst i32)
        (call $traceInstLog6 (local.get $inst)
            (call $getCodeArg (i32.const 1)) (call $getCodeArg (i32.const 2))
            (call $getCodeArg (i32.const 3)) (call $getCodeArg (i32.const 4))
            (call $getCodeArg (i32.const 5)) (call $getCodeArg (i32.const 6))
        )
    )
    ...
    (func $op0 ;; No operation
        (call $traceInst0 (i32.const 0))
        (call $addToP (i32.const 1))
    )

    ;; trace: put_structure, f/n, A1
    ...
    (func $op18 ;; put_atom
        ...
        (call $put_atom (call $getCodeArg (i32.const 1)) (call $getCodeArg (i32.const 2)))
        (call $addToP (i32.const 3))
    )

    (func $op39 ;; put_integer
        (call $traceInst2 (i32.const 39))
        (call $put_integer (call $getCodeArg (i32.const 1)) (call $getCodeArg (i32.const 2)))
        (call $addToP (i32.const 3))
    )

    (func $op40 ;; put_float
        (call $traceInst2 (i32.const 40))
        (call $put_float (call $getCodeArg (i32.const 1)) (call $getCodeArg (i32.const 2)))
        (call $addToP (i32.const 3))
    )

    (func $op19 ;; get_atom
        (call $traceInst2 (i32.const 19))
        (call $get_atom (call $getCodeArg (i32.const 1)) (call $getCodeArg (i32.const 2)))
        (if (global.get $fail)
            (then (call $backtrack))
            (else (call $addToP (i32.const 3)))
        )
    ...
    (func $op20 ;; set_atom
        ...
        (call $set_atom (call $getCodeArg (i32.const 1)))
        (call $addToP (i32.const 2))
    )
    ...
    (func $op21 ;; unify_atom
        (call $traceInst1 (i32.const 21))
        (call $unify_atom (call $getCodeArg (i32.const 1)))
        (if (global.get $fail)
            (then (call $backtrack))
            (else (call $addToP (i32.const 2)))
        )
    ...
    (func $op33 ;; switch_on_term
        (call $traceInst6 (i32.const 33))
        (call $switch_on_term (call $getCodeArg (i32.const 1)) (call $getCodeArg (i32.const 2))
            (call $getCodeArg (i32.const 3)) (call $getCodeArg (i32.const 4))
            (call $getCodeArg (i32.const 5)) (call $getCodeArg (i32.const 6))
        )
...
;;    )
    ...
    (func $put_constant (param $t i32) (param $c i32) (param $reg i32)
        (call $storeToRegister (local.get $reg) (call $applyTag (local.get $t) (local.get $c)))
    )

    (func $put_atom (param $c i32) (param $reg i32)
        (call $put_constant (global.get $TAG_ATM) (local.get $c) (local.get $reg))
    )

    (func $put_integer (param $c i32) (param $reg i32)
        (call $put_constant (global.get $TAG_INT) (local.get $c) (local.get $reg))
    )

    (func $put_float (param $c i32) (param $reg i32)
        (call $put_constant (global.get $TAG_FLT) (local.get $c) (local.get $reg))
    )

    (func $get_constant_address (param $t i32) (param $c i32) (param $addr i32)
    ...
    (func $get_constant_direct (param $t i32) (param $c i32) (param $addr i32)
        ...
        (if (i32.eq (global.get $TAG_REF) (local.get $tag))
            (then
                (call $storeToAddress (local.get $addr) (call $applyTag (local.get $t) (local.get $c)))
    ...
    (func $get_constant (param $t i32) (param $c i32) (param $reg i32)
        (local $addr i32)
        (local.set $addr (call $deref (call $tempRegisterAddress (local.get $reg))))
        (call $get_constant_direct (local.get $t) (local.get $c) (local.get $addr))
    )
    (func $get_atom (param $c i32) (param $reg i32)
        (call $get_constant (global.get $TAG_ATM) (local.get $c) (local.get $reg))
    )
    (func $get_integer (param $c i32) (param $reg i32)
        (call $get_constant (global.get $TAG_INT) (local.get $c) (local.get $reg))
    )
    (func $get_float (param $c i32) (param $reg i32)
        (call $get_constant (global.get $TAG_FLT) (local.get $c) (local.get $reg))
    )
    ...
    (func $set_atom (param $c i32)
        (call $set_constant (global.get $TAG_ATM) (local.get $c))
    )

    (func $set_integer (param $c i32)
        (call $set_constant (global.get $TAG_INT) (local.get $c))
    )

    (func $set_float (param $c i32)
        (call $set_constant (global.get $TAG_FLT) (local.get $c))
    )

    (func $unify_constant (param $t i32) (param $c i32)
        (if (i32.eq (global.get $mode) (global.get $READ_MODE))
            (then
                (call $get_constant_address (local.get $t) (local.get $c) (global.get $S))
            )
            (else ;; WRITE_MODE
    ...
    (func $unify_atom (param $c i32)
        (call $unify_constant (global.get $TAG_ATM) (local.get $c))
    )

    (func $unify_integer (param $c i32)
        (call $unify_constant (global.get $TAG_INT) (local.get $c))
    )

    (func $unify_float (param $c i32)
        (call $unify_constant (global.get $TAG_FLT) (local.get $c))
    )
...
;; switch_on_term V,A,I,F,L,S
;; switch_on_constant N,T
;; switch_on_structure N,T

    (func $switch_on_term (param $V i32) (param $A i32) (param $I i32) (param $F i32) (param $L i32) (param $S i32)
        ...
        (local.set $next
            (if (result i32)
                ...
                (then (local.get $V))
            (else (if (result i32) (i32.eq (local.get $tag) (global.get $TAG_ATM))
                (then (local.get $A))
            (else (if (result i32) (i32.eq (local.get $tag) (global.get $TAG_INT))
                (then (local.get $I))
            (else (if (result i32) (i32.eq (local.get $tag) (global.get $TAG_FLT))
                (then (local.get $F))
    ...
    (export "put_atom_opcode" (func $put_atom_opcode))
    (export "put_integer_opcode" (func $put_integer_opcode))
    (export "put_float_opcode" (func $put_float_opcode))
    (export "get_atom_opcode" (func $get_atom_opcode))
    (export "get_integer_opcode" (func $get_integer_opcode))
    (export "get_float_opcode" (func $get_float_opcode))
    (export "set_atom_opcode" (func $set_atom_opcode))
    (export "set_integer_opcode" (func $set_integer_opcode))
    (export "set_float_opcode" (func $set_float_opcode))
    (export "unify_atom_opcode" (func $unify_atom_opcode))
    (export "unify_integer_opcode" (func $unify_integer_opcode))
    (export "unify_float_opcode" (func $unify_float_opcode))
